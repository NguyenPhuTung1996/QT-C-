...
// Get new Applications List and replace to m_reoderdata
void ApplicationsModel::getApplication(int index, QString title, QString url, QString iconPath) {
    ...
    m_reoderdata[index] = applicationItem;
}

// Copy data from m_data to m_reoderdata after read file .xml
void ApplicationsModel::copy(){
    m_reoderdata = m_data;
}

// Get data from m_reoderdata
QList<ApplicationItem> ApplicationsModel::getData(){
    return m_reoderdata;
}
