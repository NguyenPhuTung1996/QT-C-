...
Item {
  ...
  // back button
    ...
    // dinh dang thoi gian, ngay thang
    QtObject {
        id: time
    ...
    Timer{
        interval: 1000     // moi 1s cap nhat 1 lan
        repeat: true
        running: true
        onTriggered: {
            time.currentTime = new Date()
            clockTime.text = time.currentTime.toLocaleTimeString(locale, "hh:mm"); // dinh dang hien thi thoi gian
            day.text = time.currentTime.toLocaleDateString(locale, "MMM. dd");      // dinh dang hien thi ngay thang
        }
